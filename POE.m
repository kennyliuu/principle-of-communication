...
R121 = input('Enter vector of desired R 1 2 values; <= 3 values ');
A = char('-','-.',':','--');
LR = length(R12);   
...
for k = 1:LR                        % Loop for various desired values of R12
    ...
    semilogy(z_dB,P_E,A(k,:)),axis([0 15 10^(-6) 1]),xlabel('E_b/N_0,dB'),ylabel('P_E');
    if k==1
...
z = 10.^(z_dB/10);                  % Convert dB to ratios
for k = 1:LR                        % Loop for various desired values of R12
    P_E=qfunc(sqrt(z*(1-R121(k))));    % Probability of error for vector of z-values
                                    % Plot probability of error versus Eb/N0 in dB
    semilogy(z_dB,P_E,A(k,:)),axis([0 15 10^(-6) 1]),xlabel('E_b/N_0,dB'),ylabel('P_E');
    if k==1
        hold on; grid % Hold plot for plots for other values of R121
    end
    end
if LR == 1 % Plot legends for R12 values
legend(['R_1_2 = ',num2str(R12(1))])
elseif LR == 2
legend(['R_1_2 = ',num2str(R12(1))],['R_1_2 = ',num2str(R12(2))])
elseif LR == 3
legend(['R_1_2 = ',num2str(R12(1))],['R_1_2 = ',num2str(R12(2))],['R_1_2 = ',num2str(R12(3))])
end
...
%function Q=qfn(x)
Q = 0.5*erfc(x/sqrt(2));
>>>>>>> a5fbd4ba2290551adac2f7852b9ea720b4d11baf
% End of script ﬁle